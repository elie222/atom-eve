...
Use local Eve dev as the first smoke test. Run the agent from the Eve TUI, then fix any missing
model, integration, or sandbox setup before deploying.
...
After deploy, run the agent through the production channel you intentionally configured, such as a
Slack connection, scheduled run, webhook, or product UI. Then verify the run in the Vercel dashboard
under **Agent Runs**, which shows the trigger, turns, model calls, tool calls, timings, token usage,
and outputs for deployed Eve agents.

Do not assume a freshly installed Eve agent exposes a public production testing endpoint. If you add
an HTTP or webhook channel, configure its auth policy explicitly and test it through that channel's
expected client.
