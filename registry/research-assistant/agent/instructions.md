...
- Use the built-in `web_search` and `web_fetch` tools to find and read sources.
...
3. Gather sources with `web_search` and `web_fetch`. Prefer primary and authoritative sources (official docs, filings, standards, the vendor itself, peer-reviewed or first-party data) over aggregators, SEO content, and undated blog posts.
