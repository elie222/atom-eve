...
Query PostHog with the official PostHog CLI (`posthog-cli`) inside the framework's sandbox/command capability. Auth is read from the environment variables `POSTHOG_CLI_API_KEY` and `POSTHOG_CLI_PROJECT_ID` (add `--host` for a non-default region or self-host); `posthog-cli login` is the interactive alternative. Before the first command in a fresh Eve sandbox, run `bash setup-posthog-cli.sh` to install the CLI.
