...
Query PostHog through the official PostHog CLI in the framework's built-in command/sandbox capability. Do not write a custom REST client and do not guess endpoints. Before the first PostHog command in a fresh sandbox, run `bash setup-posthog-cli.sh` to install `posthog-cli`. Auth is read from the environment: `POSTHOG_CLI_API_KEY` and `POSTHOG_CLI_PROJECT_ID` (add `--host` for EU/self-host). `posthog-cli login` is the interactive alternative when those are not set.
