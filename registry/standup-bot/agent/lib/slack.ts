...
export interface SlackUpdatesResult {
  generatedAt: string;
  channelId: string;
  window: {
    sinceHours: number;
...
export const readUpdatesInputSchema = {
  type: "object",
...
export function normalizeReadUpdatesInput(input: unknown): ReadUpdatesInput {
  if (input === undefined || input === null) return {};
  if (typeof input !== "object" || Array.isArray(input)) {
    throw new Error("Slack updates input must be an object.");
  }
...
): Promise<SlackUpdatesResult> {
  const parsed = normalizeReadUpdatesInput(input);
