...
Use the sandbox `bash` tool. For raw HTTP, headers, and parsing, prefer simple built-in commands such as `curl`, `node`, or runtime `fetch`. For visible copy, CTA, and rendered-page checks, drive Agent Browser: before the first Agent Browser command in a fresh sandbox, run `bash scripts/setup-agent-browser.sh`, then use commands like `npx agent-browser --session seo-audit open https://example.com`, `npx agent-browser --session seo-audit snapshot -i`, and `npx agent-browser --session seo-audit screenshot reports/assets/home.png`. Re-snapshot after every navigation because element refs expire. Do not add or rely on a custom browser wrapper tool. If a capability is unavailable, continue with the remaining checks and call out the limitation.
