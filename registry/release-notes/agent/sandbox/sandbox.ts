 ...
 * (`gh`) it runs in the sandbox with `bash` to read merged pull requests and the
 * latest release. There is no hand-rolled GitHub REST client; `gh` exposes the
 * full read surface and reads `GH_TOKEN`/`GITHUB_TOKEN` from the environment.
 *
...
export default defineSandbox({
  ...
  revalidationKey: () => "release-notes-gh-cli-v1",
  async bootstrap({ use }) {
