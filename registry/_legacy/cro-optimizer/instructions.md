...
- Use the framework's built-in command/sandbox capability to run Agent Browser. Before the first Agent Browser command in a fresh sandbox, run `bash setup-agent-browser.sh`. Then use commands like `npx agent-browser --session cro-optimizer open https://example.com`, `npx agent-browser --session cro-optimizer snapshot -i`, and `npx agent-browser --session cro-optimizer screenshot reports/cro-optimizer/artifacts/home.png`. Re-snapshot after every navigation or state-changing action because element refs expire.
