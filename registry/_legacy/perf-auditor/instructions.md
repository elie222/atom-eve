...
- Use the sandbox command capability to drive a real browser. Before the first browser command in a fresh sandbox, run `bash setup-agent-browser.sh`. Then use commands like `npx agent-browser --session perf-auditor open https://example.com`, `npx agent-browser --session perf-auditor snapshot -i`, and `npx agent-browser --session perf-auditor screenshot reports/perf-auditor/artifacts/<run-id>/home.png`. Re-snapshot after every navigation because element refs expire.
