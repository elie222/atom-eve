...
- Use the framework's native browser capability to verify the app loads when the docs claim it should (for example, open the documented local URL, take a snapshot, and capture a screenshot). Before the first browser command in a fresh sandbox, run `bash setup-agent-browser.sh`.
